...
const DEFAULT_GLOW_COLOR = '220, 0, 0';
  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      ...
      cards.forEach(card => {
        ...
        let glowIntensity = distance <= proximity ? 1 : distance <= fadeDistance ? (fadeDistance - distance) / (fadeDistance - proximity) : 0;
    ...
    <>
      <style>{`
        ...
        .mb-card--border-glow:hover {
          box-shadow: 0 4px 24px rgba(0,0,0,0.35), 0 0 30px rgba(${glowColor}, 0.15);
        }
        .mb-particle::before {
          content: '';
          position: absolute;
      ...
      <div className="mb-bento-section" ref={gridRef}>
        <div className="mb-grid">
          {cards.map((card, index) => {
            const baseClass = `mb-card ${enableBorderGlow ? 'mb-card--border-glow' : ''} mb-card-${index + 1}`;
            const cardStyle = {
              '--glow-x': '50%',
              '--glow-y': '50%',
              '--glow-intensity': '0',
            ...
            const content = (
              <>
                ...
                <div className="mb-card-body">
                  {card.title && (
                    <h3 className="mb-card-title relative z-10">{card.title}</h3>
                  )}
                  {card.description && (
                    <p className="mb-card-description relative z-10">{card.description}</p>
                  )}
